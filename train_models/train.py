...
def train(net_factory, prefix, end_epoch, base_dir,
    ...
    if net == 'PNet':
        ...
        dataset_dir_wider = os.path.join(base_dir, 'train_%s_landmark_wider.tfrecord_shuffle' % net)
        print('dataset dir wider is:', dataset_dir_wider)
        image_batch_wider, label_batch_wider, bbox_batch_wider, landmark_batch_wider = read_single_tfrecord(dataset_dir_wider, 30, net) #71701 in total
        image_batch = tf.concat([image_batch, image_batch_wider], 0, name = "concat/images")
        label_batch = tf.concat([label_batch, label_batch_wider], 0, name="concat/label")
        bbox_batch = tf.concat([bbox_batch, bbox_batch_wider], 0, name="concat/roi")
        landmark_batch = tf.concat([landmark_batch, landmark_batch_wider], 0, name="concat/landmark")
    ...
    #RNet use 3 tfrecords to get data    
        ...
        landmark_dir = os.path.join(base_dir, 'train_%s_landmark.tfrecord_shuffle' % net)
    ...
    #define placeholder
    input_image = tf.placeholder(tf.float32, shape=[config.BATCH_SIZE + 30, image_size, image_size, 3], name='input_image')
    label = tf.placeholder(tf.float32, shape=[config.BATCH_SIZE + 30], name='label')
    bbox_target = tf.placeholder(tf.float32, shape=[config.BATCH_SIZE + 30, 4], name='bbox_target')
    landmark_target = tf.placeholder(tf.float32,shape=[config.BATCH_SIZE + 30,10],name='landmark_target')
    ...
    sess.graph.finalize()
    try:
        log_f = open('../logs/%s_train_log.txt' % net, 'w')
        for step in range(MAX_STEP):
            ...
            if (step+1) % display == 0:
                ...
                log_f.write("%s : Step: %d/%d, accuracy: %3f, cls loss: %4f, bbox loss: %4f,Landmark loss :%4f,L2 loss: %4f, Total Loss: %4f ,lr:%f \n" % (
                datetime.now(), step+1,MAX_STEP, acc, cls_loss, bbox_loss,landmark_loss, L2_loss,total_loss, lr))
