...
def getDataFromTxt2(txt,data_path, with_landmark=True):
    """
        Generate data from txt file
        return [(img_path, bbox, landmark)]
    ...
    for line in lines:
        if line.find("jpg") > 0:
            img_path = os.path.join(data_path, line.strip().split(' ')[1]).replace('\\', '/')  # file path
            continue
        line = line.strip()
        components = line.split()
        if float(components[6]) == -1: # remove dropped
            continue

        # bounding box, (x1, y1, x2, y2)
        #bbox = (components[1], components[2], components[3], components[4])
        bbox = [float(components[0]), float(components[1]), float(components[0]) + float(components[2]),  float(components[1]) + float(components[3])]
        #bbox = [float(_) for _ in bbox]
        bbox = list(map(int,bbox))
        # landmark
        if not with_landmark:
        ...
        landmark = np.zeros((5, 2))
        for index in range(0, 5):
            rv = (float(components[(index+1)*3+1]), float(components[(index+1)*3+2]))
            landmark[index] = rv
        #normalize
        '''
        for index, one in enumerate(landmark):
