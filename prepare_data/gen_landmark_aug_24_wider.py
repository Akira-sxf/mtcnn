...
import sys
sys.path.insert(0,'..')
from prepare_data.BBox_utils import getDataFromTxt2,processImage,shuffle_in_unison_scary,BBox
from prepare_data.Landmark_utils import show_landmark,rotate,flip
import random
import tensorflow as tf
import numpy.random as npr
dstdir = "../data/24/train_RNet_landmark_aug_wider"
OUTPUT = '../data/imglists_noLM/RNet'
...
def GenerateData(ftxt, data_path,net,argument=False):
    if net == "PNet":
        ...
        return
    image_id = 0
    f = open(join(OUTPUT,"landmark_%s_aug_wider.txt" %(size)).replace('\\', '/'),'w')
